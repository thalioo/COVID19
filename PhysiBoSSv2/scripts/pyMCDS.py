...
class Settings(object):
    def __init__(self,xml_file="settings.xml",output_path="."):
        self.data = self._read_xml(xml_file, output_path)
    def _read_xml(self, xml_file, output_path='.'):

        filepath = output_path +"./" + xml_file
        tree = ET.parse(filepath)
        root = tree.getroot()
         
        Settings = {}
        me_node= root.find("microenvironment_setup")
        var_children = me_node.findall('variable')
        for si, species in enumerate(var_children):
            species_name = species.get('name')
            init_con_node = species.find("initial_condition")
            species_i_condition = init_con_node.text
            Settings['variables'] = {species_name:species_i_condition}

        up_node = root.find("user_parameters")
        div_node = up_node.find("div_initialization")
        mov_node = up_node.find("multiplicity_of_infection").text
        Settings["user_parameters"] = {"multiplicity_of_infection":mov_node}
        # tree.write(filepath)
        return(Settings)
